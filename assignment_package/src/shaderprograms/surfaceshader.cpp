...
SurfaceShader::SurfaceShader(OpenGLContext *context)
    ...
    unifModel(-1), unifModelInvTr(-1), unifView(-1), unifProj(-1), unifCamP(-1)
{}
...
void SurfaceShader::setupMemberVars()
{
    ...
    unifCamP       = context->glGetUniformLocation(prog, "u_CamP");
...
void SurfaceShader::setCameraPostion(const glm::vec3 &camP) {
    useMe();
    if (unifCamP != -1) {
        context->glUniform3fv(unifCamP, 1, &camP[0]);
    }
}
